    ...
    <el-table v-loading="loading" :data="ipList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['app:ip:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listIp, getIp, delIp, addIp, updateIp } from "@/api/app/ip";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除ip白名单编号为"' + _ids + '"的数据项？').then(function() {
    return delIp(_ids);
  }).then(() => {
    getList();
